...
TilePather::TilePather(QWidget* parent)
	...
	for (const auto& i : terrain.tileset_ids) {
		const uint8_t pathing = tilesets.terrain_texture(i)->get_tile_pathing();
...
void TilePather::save_tiles() {
	...
	for (const auto& [tile_id, options] : pathing_options) {
		...
		TerrainTexture* texture = tilesets.terrain_texture(tile_id);
		if (mask != texture->get_tile_pathing()) {
			changed_ids.insert(terrain.ground_texture_to_id(tile_id));
		}
	...
	if (!changed_ids.empty()) {
		for (int i = 0; i < terrain.width; i++) {
			for (int j = 0; j < terrain.height; j++) {
				if (!changed_ids.contains(terrain.real_tile_texture(i, j))) {
					continue;
				}

				const int x0 = std::max(i * 4 - 2, 0);
				const int y0 = std::max(j * 4 - 2, 0);
				const int x1 = std::min(i * 4 + 2, pathing_map.width);
				const int y1 = std::min(j * 4 + 2, pathing_map.height);

				for (int y = y0; y < y1; y++) {
					for (int x = x0; x < x1; x++) {
						pathing_map.pathing_cells_static[y * pathing_map.width + x] =
							terrain.get_terrain_pathing(x, y, true, true, true, tilesets);
					}
				}
			}
		}
